...
// query to get messages from messages table                        select * from prod_cat where category = "tv";
$prod_tele = 'TV';
$q = "SELECT * FROM prod_cat where category = '$prod_tele' LIMIT $start, $per_page";
$query = $pdo->prepare($q);
$query->execute();
...
$count_query = "SELECT * FROM prod_cat where category = '$prod_tele'";
...
<body>
  ...
  <section class="light">
    <div class="container py-4">
      <h1 class="headprod">Televisions</h1>
      ...
      <?php foreach ($result as $data) : ?>
        <article class="postcard dark red geeks">
          ...
          <div class="postcard__text">
            <br>
            <h1 class="postcard__title red"><a href="index.php?page=product&product_id=<?= $data['product_id'] ?>"><?php echo $data['name']; ?></a></h1><br>
            <div>
              <h4>Price : &#8377;<?php echo $data['sale_price'];  ?></h4>
            </div>
            <div>
              <h6>MRP : <strike>&#8377;<?php echo $data['mrp_price'];  ?></strike></h6>
            </div><br>
            <div>
              <h6>Display Type : <?php echo $data['screen_resolution'];  ?></h6>
            </div> <br>
            <div>
              <h6>Display Size : <?php echo $data['size'];  ?></h6>
            </div>
            <br>
          </div>
        </article>
